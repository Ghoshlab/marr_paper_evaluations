...
#outlier.index<- array(0, dim=c(6))
#for(i in 1:6)
#{
#  outlier.index[i]<- which(rownames(metabo_t_p2)==outliers[i])
#}
...
#rownames(metabolon) <- metabolon$X
#metabolon$X <- NULL
#library(rlang)
#library(tidyverse)
##metabolon <- as.data.frame(t(as.matrix(metabolon)))
data.table::setDT(raw.data, keep.rownames = TRUE)

# metabolon pipeline
#library(rlang)
#library(tidyverse)
#tidied_metabolon <- ms_tidy(raw.data, met_id = "rn",
# col_names = "subject_id")

#summarized_metabolon = ms_summarize(tidied_metabolon, met_id = "rn",
#  cvmax = 0.50, min_proportion_present = 1/3,
# missing_val = 0)
library(rlang)
library(tidyverse)
library(tibble)
#filtered_metabolon = ms_filter(summarized_metabolon, filter_percent=0.8)
#filtered_metabolon<- summarized_metabolon
#options(warn=0)
#library(pcaMethods)
#imputed_metabolon = ms_impute(filtered_metabolon, imputeMethod ="bpca", k_bpca = 10,n_pcs =5)
imputed_metabolon = readRDS("../data/Metabolon_bpca_object.rds")

normalized_metabolon = ms_normalize(imputed_metabolon, normalizeMethod ="median",
...
save(saved_metabolon,file="../saved_objects/Metabolon_bpca_median_load.R")
