#####  This R file contains functions needed to generate bivariate rank data and to perform the MaRR procedure ####
library(mvtnorm)
...
#         x.idr = vector of signals suitable for use with the i dr package
get.ranks.fixedn1 <- function(n=1000, n1=650,mu1,mu2,v1,v2,rho1) 
{
  Zijs = matrix(NA,2,n)
  n0 <- n-n1
  ks = sample(c(rep(0,n0),rep(1,n1)))
  which.sig = which(ks==1); which.notsig = which(ks==0)
  Zijs[,which.sig] = t(rmvnorm(n1,mean=c(mu1,mu1),
                             sigma =matrix(c(v1,(sqrt(v1*v1)*rho1),
                                             (sqrt(v1*v1)*rho1),v2),2,2)))
  Zijs[,which.notsig] = t(rmvnorm(n0,mean=c(mu2,mu2),
                                  sigma =matrix(c(v2,0,
                                                  0,v2),2,2)))
